...
namespace ichi
{
	...
	ID3D12Resource* create_gray_gradation_texture(device* device)
	{
		D3D12_HEAP_PROPERTIES texHeapProp{};
		texHeapProp.Type = D3D12_HEAP_TYPE_CUSTOM;//特殊な設定なのでdefaultでもuploadでもなく
		texHeapProp.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_WRITE_BACK;//ライトバックで
		...
		if (FAILED(result->WriteToSubresource(0, nullptr, data.data(), 4 * sizeof(unsigned int), sizeof(unsigned int) * static_cast<UINT>(data.size()))))
		{
			if (result)
				result->Release();
			return nullptr;
	...
	gray_gradation_texture_resource::~gray_gradation_texture_resource()
	{
		if (m_resource)
			m_resource->Release();
	}
	...
	{
		m_resource = create_gray_gradation_texture(device);
		if (m_resource)
			return true;
		else
			return false;
	...
	ID3D12Resource* gray_gradation_texture_resource::get()
	{
		return m_resource;
	}
